...
| # | Milestone | Repo Status | Canonical Paths | Notes |
|---|-----------|-------------|-----------------|-------|
| 13 | Compatibility profile system | STARTED | `src/common/compatibility_profile.*`, `src/common/prefix_preset.*`, `profiles/`, `tests/unit/compatibility_profile_tests.cpp`, `tests/unit/prefix_preset_tests.cpp` | Parser, runtime knobs, Wine 11+/Mono policy, API-route backend intent, install/setup policy, prefix-preset validation, and auto-selection helper are checked in |
| 14 | Runtime launcher/product integration | STARTED | `src/common/runtime_launch_plan.*`, `src/common/runtime_launch_command.*`, `tools/`, `launcher/`, future runtime glue | Launch-plan builder, launch-command materializer, preview tool, and wrapper-script export are checked in, now including install/setup metadata, managed-prefix resolution, and Wine/API-route env export; the launcher now consumes bundled project-status, compatibility-catalog, runtime-plan previews, portable imported runtime-bundle manifests, in-app checklist/setup/launch/lint/catalog previews, guided runtime actions, reveal/open/export actions, and one-click Prepare Title/launch actions for imported runtime bundles |
| 15 | Backend breadth and fallback strategy | STARTED | `src/common/compatibility_profile.*`, `src/common/runtime_launch_plan.*`, `profiles/`, future runtime/backend glue | DX11/DX12/Vulkan route policy, Wine 11+ floor, and Mono requirement intent are now expressed in shared runtime policy; real runtime wiring is still pending |
| 16 | Compatibility database and installer flow | STARTED | `profiles/`, `src/common/compatibility_catalog.*`, `src/common/prefix_preset.*`, `tools/`, launcher, future runtime tooling | Compatibility catalog/reporting plus merged prefix-preset setup intent are checked in; known-title onboarding now auto-resolves managed prefixes and drives guided title preparation, while fuller installer/runtime execution remains pending |
...
| Wine/runtime route policy validation | IN PLACE | Checked-in profiles now validate Wine 11+ floor/preference, Mono requirement intent, and DX11/DX12/Vulkan backend-route policy in host tests |
...
| Runtime tester bundle export | IN PLACE | `tools/mvrvb_runtime_bundle_builder` and `scripts/export_runtime_bundle.ps1` package launch-plan, setup-script, compatibility-catalog, and lint outputs into a single handoff directory without Mac hardware, now with resolved managed-prefix metadata when no explicit prefix path is supplied |
...
| Launcher known-title onboarding | IN PLACE | `launcher/CompatibilityCatalog.swift`, `launcher/BridgeViewModel.swift`, and `launcher/ContentView.swift` now turn the bundled compatibility catalog into a browsable known-title card with app-managed prefix summaries and guided Prepare Title onboarding for selected titles |
| Launcher starter-bundle generation | IN PLACE | `launcher/BridgeViewModel.swift`, `launcher/ContentView.swift`, `launcher/setup.sh`, and `.github/workflows/build.yml` now bundle the runtime-bundle helper plus checked-in `profiles/` tree so the app can generate, import, and prepare starter runtime bundles directly when those resources are present |
| Runtime launch-plan builder | IN PLACE | `src/common/runtime_launch_plan.*` resolves profiles plus checked-in prefix presets into backend/install/env/args/runtime policy, managed-prefix root/path decisions, can persist and reload schema-versioned JSON plans, and can emit Markdown setup checklists in host tests |
...
| Runtime setup-command materializer | IN PLACE | `src/common/runtime_setup_command.*` turns install policy into bootstrap actions, manual follow-up notes, and bash/PowerShell setup scripts without Mac hardware, defaulting to the resolved managed prefix when no explicit override is given |
| Runtime plan preview tool | IN PLACE | `tools/mvrvb_runtime_plan_preview` resolves a profile query into a concrete launch summary, JSON launch-plan payload, Markdown setup checklist, launch wrapper script, or setup/bootstrap script, can persist those outputs to disk, can reload exported JSON plans, and can compute managed prefixes without Mac hardware |
