...
| Launcher runtime-bundle manifest preview | IN PLACE | `launcher/RuntimeBundleManifest.swift`, `launcher/BridgeViewModel.swift`, and `launcher/ContentView.swift` now import `bundle-manifest.json` exports and load the referenced `launch-plan.json` for in-app preview |
