...
| Runtime plan preview tool | IN PLACE | `tools/mvrvb_runtime_plan_preview` resolves a profile query into a concrete launch summary, JSON launch-plan payload, Markdown setup checklist, or bash/PowerShell wrapper script, can persist those outputs to disk, and can reload exported JSON plans without Mac hardware |
