...
| Launcher known-title onboarding | IN PLACE | `launcher/CompatibilityCatalog.swift`, `launcher/BridgeViewModel.swift`, and `launcher/ContentView.swift` now turn the bundled compatibility catalog into a browsable known-title card with starter runtime-bundle command generation for selected titles |
| Launcher starter-bundle generation | IN PLACE | `launcher/BridgeViewModel.swift`, `launcher/ContentView.swift`, `launcher/setup.sh`, and `.github/workflows/build.yml` now bundle the runtime-bundle helper plus checked-in `profiles/` tree so the app can generate and import starter runtime bundles directly when those resources are present |
